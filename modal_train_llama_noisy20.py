"""
Run ARC-BPO training for Llama-3-8B with 20% noisy preference labels on Modal.

This is for the label-noise robustness setting:
  randomly swap chosen/rejected in 20% of training preference pairs.

Full run, public HF adapter upload:
  modal run --detach modal_train_llama_noisy20.py::main \
    --hf-repo-id ducthang1703/llama3-8b-arc-bpo-lora-noise20-full

Smoke test:
  modal run --detach modal_train_llama_noisy20.py::main \
    --n-examples 128 \
    --hf-repo-id ducthang1703/llama3-8b-arc-bpo-lora-noise20-smoke-128

Inspect saved Modal volume outputs:
  modal run modal_train_llama_noisy20.py::ls_outputs
"""

import modal


APP_NAME = "tbpo-arc-bpo-llama-noisy20-train"
RUN_VERSION = "arc-bpo-llama3-noisy20-h100x4-v1"

REPO_DIR = "/root/arc_bpo"
VOLUME_ROOT = "/vol/output"
OUTPUT_DIR = f"{VOLUME_ROOT}/train_runs_llama_noisy20"
GPU_CONFIG = "H100:4"
...
def main(
    hf_repo_id: str = "ducthang1703/llama3-8b-arc-bpo-lora-noise20-full",
    hf_private: bool = False,
    batch_size: int = 64,
    grad_accum: int = 4,
    n_epochs: int = 1,
    ...
    print("[RESULTS] Inspect outputs with: modal run modal_train_llama_noisy20.py::ls_outputs")
