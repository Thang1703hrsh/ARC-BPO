"""
Run ARC-BPO training for Qwen2.5-7B-Instruct on Modal.
...
Launch training (detached, recommended -- training is long):
  modal run --detach modal_train_qwen.py::main

Override any hyperparameter, e.g. a quick smoke test:
  modal run --detach modal_train_qwen.py::main --n-examples 64 --batch-size 16 --no-do-first-eval

Upload the final LoRA adapter to Hugging Face:
  modal run --detach modal_train_qwen.py::main --n-examples 10000 --batch-size 64 --grad-accum 4 --n-eval-examples 0 --no-do-first-eval --hf-repo-id "ducthang1703/qwen25-7b-instruct-arc-bpo-lora-10k" --no-hf-private

Inspect what landed in the volume afterwards:
  modal run modal_train_qwen.py::ls_outputs
"""

import modal


APP_NAME = "tbpo-arc-bpo-qwen-train"
RUN_VERSION = "arc-bpo-qwen25-7b-h100x4-hub-upload-v1"
...
OUTPUT_DIR = f"{VOLUME_ROOT}/train_runs_qwen"

# Multi-GPU because train.py uses FSDP: world_size = torch.cuda.device_count().
# Keep this aligned with the Llama/Mistral Modal scripts unless you intentionally
# want a cheaper/smaller Modal run.
GPU_CONFIG = "H100:4"
...
# HF secret is required for private/gated access and optional post-training
# uploads. The Qwen base model is public, but keeping this secret here makes the
# upload path match the other Modal training scripts.
_SECRETS = [
...
def train(
    # --- ARC-BPO loss hyperparameters (match arc_bpo_qwen.sh) ---
    beta: float = 0.1,
    delta_star: float = 2.0,
    arc_t: float = 2.0,
    kappa: float = 2.0,
    ...
    # --- data / optimization ---
    model_config: str = "qwen2_5_7b_instruct",
    ...
    if hf_repo_id:
        ...
        upload_folder(
            ...
            commit_message=f"Upload ARC-BPO Qwen checkpoint ({RUN_VERSION})",
        )
...
def ls_outputs():
    """List the Qwen training run directories saved in the volume."""
    import os
...
def main(
    ...
    """Launch ARC-BPO Qwen training on Modal (mirrors arc_bpo_qwen.sh)."""
    if smoke:
        if n_examples == 0:
            n_examples = 32
    ...
    print("[RESULTS] Inspect outputs with: modal run modal_train_qwen.py::ls_outputs")
