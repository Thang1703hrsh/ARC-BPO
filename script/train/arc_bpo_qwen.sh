...
# ARC-BPO on Qwen2.5-7B-Instruct from experiments.md.
...
MODEL_CONFIG="${MODEL_CONFIG:-qwen2_5_7b_instruct}"
...
# 2.0 default to avoid over-pushing the policy away from its strong init, and
# keep beta at 0.1 so the reference anchor stays meaningful.
BETA="${BETA:-0.1}"
...
# NOTE: no advantage proxy is produced by the pipeline, so use_advantage_shape
# would fall back to uniform. Kept false to reflect actual behavior.
...
TRAIN_LOG="${TRAIN_LOG:-${LOG_DIR}/arc_bpo_qwen_${RUN_STAMP}.log}"
