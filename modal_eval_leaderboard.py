...
Model: tp140205/mistral-7b-v0.1-a-tbpo-ultrafeedback  (checkpoint: step_32000)

Step 1 — launch (detached, no heartbeat timeout):
  modal run --detach modal_eval_leaderboard.py::main

Step 2 — print results after job finishes (~2-3h):
  modal run modal_eval_leaderboard.py::results
"""

import modal

app = modal.App("fswift-eval-leaderboard")
...
# ── Model config ──────────────────────────────────────────────
HF_REPO      = "tp140205/mistral-7b-v0.1-a-tbpo-ultrafeedback"
CHECKPOINT   = "step_32000"
MODEL_LABEL  = "mistral-7b-tbpo-step32000"
...
def download_model():
    ...
    print(f"[DOWNLOAD] {HF_REPO} / {CHECKPOINT}")
    # Download only the step_32000 subfolder
    tmp = f"{VOLUME_ROOT}/_tmp_{MODEL_LABEL}"
    snapshot_download(
        repo_id=HF_REPO,
        allow_patterns=[f"{CHECKPOINT}/**", f"{CHECKPOINT}/*"],
        local_dir=tmp,
        token=os.environ.get("HF_TOKEN"),
    )

    # Move checkpoint subfolder to MODEL_LOCAL
    import shutil
    src = os.path.join(tmp, CHECKPOINT)
    if os.path.isdir(src):
        shutil.copytree(src, MODEL_LOCAL)
        shutil.rmtree(tmp, ignore_errors=True)
        print(f"[DOWNLOAD] Saved to {MODEL_LOCAL}")
    else:
        # Fallback: snapshot had no subfolder structure → use as-is
        shutil.move(tmp, MODEL_LOCAL)
        print(f"[DOWNLOAD] Saved to {MODEL_LOCAL} (no subfolder found)")
